...
static int BootCtrl_BuildAttemptIncrementImage(boot_ctrl_t *out, const boot_ctrl_t *current)
{
    ...
    if (current->boot_attempts == UINT32_MAX) {
         return 0;
    }
...
int BootCtrl_BootloaderWrite_Precheck(const boot_ctrl_t *current)
{
    if (!current) {
        return 0;
    }
    ...
    if (!BootCtrl_BuildAttemptIncrementImage(&g_bootctrl_confirm_image, current)) {
        return 0;
    }
