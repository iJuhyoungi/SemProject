...
# P-4b: DMA 송신 구현 (TCD + DMAMUX + DER)

## 설정 순서 (LPSPI1_Send_DMA)
- CCM_CCGR5 |= (3<<6) — eDMA/DMAMUX 클럭 게이트 ON (CG3)
- TCD 채널0: SADDR=&buf, SOFF=1, ATTR=0(8bit), NBYTES=1, DADDR=&TDR, DOFF=0, CITER=BITER=n, CSR=DREQ
- DMAMUX_CHCFG0 = (1<<31)|14 — LPSPI1 TX(소스14)를 채널0에 연결 + enable
- LPSPI DER[TDDE]=1 — FIFO 빌 때 DMA 요청 발생
- EDMA_SERQ = 0 — 채널0 요청 수신 enable (출발 스위치)
- 검증: DMA CSR = 0x88 (DONE bit7 + DREQ bit3), `DMA OK`

## 디버깅 교훈 2종 (둘 다 흔한 베어메탈 함정)
- **클럭 안 켜면 fault 가 아니라 조용한 hang**: `CCM_CCGR5 != (3<<6)` 오타(=비교, no-op)로 게이트 OFF → 꺼진 eDMA 레지스터 접근 시 AIPS 버스가 완료를 못 해 CPU 정지. HardFault 안 뜨고 멈춤. `-Wall` 의 "statement has no effect" 경고 놓침.
- **레지스터 접근 폭 = 실제 폭**: TCD 의 16-bit 필드(SOFF/ATTR/DOFF)를 uint32_t 로 정의 → ATTR(@0x06, 4정렬 아님)에 32bit 접근 → UNALIGNED UsageFault(CFSR bit24). Device 메모리는 정렬 안 맞는 접근 무조건 fault. + 4정렬이어도 32bit 면 옆 16-bit 필드 덮어씀. → RM "Width(In bits)" 그대로 타입 맞추기.

---

# P-4c: 완료 인터럽트 + polling/IRQ/DMA 3단 비교

## 완료 인터럽트 배선 (IRQ 때 3관문과 동일 패턴)
- TCD_CSR = DREQ | INTMAJOR (0x0A) — 메이저 루프(전송 전체) 완료 시 IRQ 발생
- DMA0 = IRQ 0 (RM Table 4-2, eDMA ch0) → 벡터 인덱스 16(=16+0), startup 정적 수정
- NVIC_ISER0 bit0 — IRQ 0 enable
- DMA0_IRQHandler: EDMA_CINT=0 으로 INT 플래그 클리어 + done 플래그 set

## ★ read-back 교훈 — spurious 재진입 (dma irq count 2→1)
- 증상: 완료 인터럽트는 전송당 1번이라 count=1 이어야 하는데 **2** 나옴.
- 원인: ISR 의 `EDMA_CINT=0` write 가 **버퍼링**돼 ISR 리턴 시점까지 미반영 → NVIC 가 아직 pending 으로 보고 ISR 1회 더 진입 → count=2. (고전적 ARM 함정: write-buffered peripheral flag clear)
- 해결: 클리어 **직후 read-back** 으로 write 강제 완료.
```c
void DMA0_IRQHandler(void)
{
    EDMA_CINT = 0;        /* INT 플래그 클리어 */
    (void)EDMA_INT;       /* read-back: 클리어를 리턴 전에 반영 (재진입 방지) */
    g_dma_irq_count++;
    g_dma_done = 1;
}
```
- 일반화: peripheral 인터럽트 플래그 clear 후 같은 페리페럴 레지스터 read-back(또는 DSB)로 ISR 리턴 전 반영 보장.

## 3단 CPU 비교 결과 (40 byte, DWT CYCCNT)
| 방식 | 전송 중 main | 인터럽트 횟수 |
|---|---|---|
| polling | 전송 내내 묶임 (~10만 cyc, 데이터에 비례) | 0 |
| IRQ | 자유 (kick 후) | 3 (chunk당) |
| DMA | 자유 (kick 후) | **1 (전송당)** |
- 핵심: poll 은 데이터 크기에 비례해 CPU 점유 증가 / IRQ·DMA 는 고정 kick 후 main 자유. DMA 는 인터럽트도 전송당 1번.
- 주의: kick 사이클 절대값은 인라인 여부에 흔들림(리팩으로 9→7840) → 비교는 *스케일링 특성*으로.
