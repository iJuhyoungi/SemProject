...
## 보호 계층 — 각각 다른 위협을 담당합니다

| 계층 | 무엇을 막는가 | 검증 키 | 비용 |
|---|---|---|---|
| **무결성** (CRC, SHA) | 우연 손상, 단순 변조 | 없음 | μs ~ ms |
| **인증** (이미지의 RSA) | 위조 (다른 사람이 만든 이미지) | Stage 2 는 root, App 은 release | 약 1.4 초 (modexp) |
| **Anti-rollback** (`min_acceptable_version`) | downgrade attack (옛 취약 펌웨어) | 메타데이터 안의 정책 | μs (정수 비교) |
| **Atomic** (이중 메타데이터의 RSA) | 메타데이터 변조, 업데이트 중 전원 끊김 | root | 약 1.4 초 × 2 sector |
| **Measured boot** (정책 안의 App 측정값) | mix-and-match — 부품은 진짜인데 세대가 어긋난 조합 | 정책 서명이 측정값까지 덮음 | μs (상수시간 32 byte 비교) |
| **키 계층** (root 서명 인증서) | release 키 유출 시에도 교체 가능하게 | root | 약 1.4 초 (인증서 1회) |
| **키 폐기** (`min_key_version`) | 유출된 옛 키 + 진짜 옛 인증서의 롤백 | 정책 안의 최소 세대 | μs (정수 비교) |
| **Fault injection 내성** | 글리치 1회로 판정을 뒤집는 공격 | — | 검증 2회 실행 (부팅 시간 약 2배) |

각 계층은 아래 계층이 뚫려도 독립적으로 동작합니다. 예를 들어 서명이 유효하더라도 측정값이
정책과 다르면 거부되고, 인증서가 유효하더라도 세대가 낡았으면 거부됩니다.
...
- **키 인증서 invalid**: halt. App 을 아예 시도하지 않습니다 — 검증할 키를 믿을 수 없기 때문입니다.
- **키가 폐기됨** (`key_version < min_key_version`): halt.
- **두 번의 검증 결과가 불일치**: 해당 슬롯 거부 후 fallback.
- **측정값이 정책과 불일치**: 해당 슬롯 거부 후 fallback.
- **점프 직전 재판정 실패**: `jump_to_image()` 가 점프하지 않고 복귀 → fallback 또는 halt.
