...
#include "flexspi_nor_config.h"
  ...
  /*                                                                                                                                                                                       
   * bootloader 전용 low-level flash backend (ROM API wrapper)
   *                                                          
   * Mission 2 의 핵심: 모든 함수를 .ramfunc 에 두어 ITCM 에서 실행.
   * 이렇게 해야 flash erase/program 동안 CPU 가 FLASH 를 fetch 하려는                                                                                                                     
   * 충돌을 피할 수 있음.                                                                                                                                                                  
   *                                                                                                                                                                                       
   * 전제:                                                                                                                                                                                 
   * - shared/src/flexspi_nor_config.c 의 qspi_flash_config 심볼 존재                                                                                                                      
   * - .ramfunc 섹션이 ITCM 에 매핑되어 있음 (bootloader.ld 참고)                                                                                                                          
   */ 
...
volatile uint32_t g_boot_dbg = 0;
volatile uint32_t g_boot_dbg_status = 0;
volatile uint32_t g_boot_dbg_sts0_before=0;     /* erase 직전 STS0 스냅샷 */
volatile uint32_t g_boot_dbg_arbidle_wait=0;    /* erase 직전 ARBIDLE 대기 횟수 */
...
int BootCtrl_LowLevel_EraseSector(uint32_t address)
{
    ...
    g_boot_dbg_sts0_before = BOOTCTRL_FLEXSPI_STS0;
