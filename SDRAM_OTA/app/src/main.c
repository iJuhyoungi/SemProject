...
uint8_t dma_tx_buffer[4096] __attribute__((section(".ocram_test"), aligned(32)));
uint8_t dma_rx_buffer[4096] __attribute__((section(".ocram_test"), aligned(32)));

/* 🚀 GCC 최적화 대응용 수제 memset 함수 추가 */
...
int main(void)
{
    ...
    // MPU_Init(); /* 🚀 MPU 초기화 (캐시와 DMA가 싸우는지 확인하기 위함) */
    ...
    UART1_SendString("[1.1] About to write TX/RX buffers...\r\n");
    for (volatile uint32_t k = 0; k < 50000000; k++) {
    __asm("nop");
    }
    UART1_SendString("[1.15] Still alive after delay.\r\n");
    
    /* 1. 송신 버퍼에 테스트 데이터 꽉꽉 채우기 (CPU가 기록) */
    FillBuffers();
    
    UART1_SendString("[1.2] Buffer write done.\r\n");
